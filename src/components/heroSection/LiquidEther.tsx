...
class ShaderPass {
  ...
  update(..._args: unknown[]): void {
    const common = _args[0] as CommonClass;
...
class Advection extends ShaderPass {
  ...
  update(common: CommonClass, args?: UpdateArgs): void {
...
class ExternalForce extends ShaderPass {
  ...
  update(common: CommonClass, Mouse: MouseClass, args?: UpdateArgs): void {
    const props = args || {};
...
class Simulation {
  ...
  update(): void {
    ...
    this.externalForce.update(this.common, this.mouse, {
      cursor_size: this.options.cursor_size,
    ...
    if (this.options.isViscous) {
      const viscousResult = this.viscous.update(this.common, {
        viscous: this.options.viscous,
...
class WebGLManager {
  ...
  constructor(
    ...
    this._onVisibility = () => {
      ...
      if (hidden) {
        this.pause();
      } else if (props.isVisibleRef.current) {
        this.start();
      }
    };
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const applyOptionsFromProps = (): void => {
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      const mouseDriver = webgl.autoDriver.mouse;
      if (mouseDriver) {
        mouseDriver.autoIntensity = autoIntensity;
        mouseDriver.takeoverDuration = takeoverDuration;
      }
    }
    if (resolution !== prevRes) sim.resize();
  }, [
